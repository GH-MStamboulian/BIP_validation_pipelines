...
##  Table 5: LoD tables
###############################
#these two files are the same as the old one.
  ...
  ##  detection == Guardant360 CDx for 74 genes
  ...
  mutation2plot = unique(snv_table$mutation)
  pool2plot = snv_table$Type[match(mutation2plot, snv_table$mutation)]
  ...
  for(ng in c("5 ng", "30 ng")) {
    ...
    newZ = data.frame(target_maf = seq(0, 0.02, 0.00001)); #uncommented in the original
    #newZ = data.frame(target_maf = seq(2, 3, 0.001)); #commented in the original
  ...
  mutation2plot = unique(indel_table$mutation)
  pool2plot = indel_table$Type[match(mutation2plot, indel_table$mutation)]
  lod.plots = list()
  ...
  for(ng in c("5 ng", "30 ng")) {
    ...
    target_maf2plot = expected.maf[expected.maf$ng_seq == ng,] %>% 
      ...
      dplyr::right_join(du[,c("ng_seq", "Dilution", "call", "mutation", "Type")], 
                        by = c("ng_seq", "Dilution", "mutation"))
    iMissing = which(is.na(du$target_maf) ); #gets the index of the riws with NA values of the target_maf
    du$call = 1;
    du$call[iMissing] = 0;
    du$target_maf[iMissing] = target_maf2plot$target_maf[iMissing];
    ...
    newZ = data.frame(target_maf = seq(0, max(dup$target_maf, na.rm=T), 0.00001)); #uncommented in the original
    #newZ = data.frame(target_maf = seq(2, 3, 0.001)); #commented in the original
    newZ$pdet <- predict(m, newdata = newZ, type = "response")
    newZ$se <- predict(m, newdata = newZ, type = "response", se.fit=TRUE)$se.fit
    newZ$upperL = pmin(1.0, newZ$pdet + newZ$se * 2)
  ...
  mutation2plot = unique(cna_table$mutation)
  pools2plot = cna_table$Type[match(mutation2plot, cna_table$mutation)]
  ng_seq2plot = unique(cna_table$ng_seq)
  
  for(mut in mutation2plot) {
    for(ng in ng_seq2plot) {
      
      pool = pools2plot[mut == mutation2plot]
      d = cna_table %>% filter(mutation == mut & ng_seq == ng)
      ...
      #newZ = data.frame(titration = seq(0, 1, 0.001)); #commented in the original
      newZ = data.frame(target_maf = seq(2, 3, 0.001)); #uncommented in the original
      newZ$pdet <- predict(m, newdata = newZ, type = "response")
      newZ$se <- predict(m, newdata = newZ, type = "response", se.fit=TRUE)$se.fit
      newZ$upperL = pmin(1.0, newZ$pdet + newZ$se * 2)
      ...
      if(mut %in% detection.rate.final$mut_key) {
        ...
        lvls = paste0("Level", 1:5)
        nValid = sum(dd[,lvls] >= 0.1 & dd[,lvls] <= 0.9)
        if(nValid < 2) { ##this should have been updated to 3 as per FDA requirement
  ...
  template = type %>% 
    filter(!is.na(Dilution)) %>%
    dplyr::select(c("run_sample_id", "Dilution", "ng_seq", "Type"))
  mutation2plot = unique(fusion_table$mutation)
  dilution2plot = sort(unique(type$Dilution))
  pools2plot = fusion_table$Type[match(mutation2plot, fusion_table$mutation)]
  ng_seq2plot = unique(fusion_table$ng_seq)
  ...
  for(mut in mutation2plot) {
    for(ng in ng_seq2plot) {
      ...
      newZ = data.frame(target_maf = seq(0, max(d$target_maf, na.rm=T), 0.00001)); #uncommented in the original
      #newZ = data.frame(target_maf = seq(2, 3, 0.001)); #commented in the original
      newZ$pdet <- predict(m, newdata = newZ, type = "response")
      newZ$se <- predict(m, newdata = newZ, type = "response", se.fit=TRUE)$se.fit
      newZ$upperL = pmin(1.0, newZ$pdet + newZ$se * 2)
  ...
  lod.table.part1
##not giving the same results as original reports  1st
  write.table(lod.table.part1, file=file.path(resdir, "Table5.LoD.part1.G360CDx.74genes.tsv"), sep="\t", col.names=T, row.names=F, quote=F)
  save(lod.table.part1, file=file.path(resdir, "Table5.LoD.part1.G360CDx.74genes.RData"))
  ...
  #lod.table.part2
  write.table(lod.table.part2, file=file.path(resdir, "Table5.LoD.part2.G360CDx.74genes.tsv"), sep="\t", col.names=T, row.names=F, quote=F)
  save(lod.table.part2, file=file.path(resdir, "Table5.LoD.part2.G360CDx.74genes.RData"))
  ...
  lod.table.final
  write.table(lod.table.final, file=file.path(resdir, "Table5.LoD.G360CDx.74genes.tsv"), sep="\t", col.names=T, row.names=F, quote=F)
  save(lod.table.final, file=file.path(resdir, "Table5.LoD.G360CDx.74genes.RData"))
  
  save(glm.models, variants.df, file=file.path(resdir, "glm.models.LoD.G360CDx.74genes.200128.RData"))
  ...
  pdf(file=file.path(resdir, "FIGURE2.LoD_PLOTS_SNV.G360CDx.74genes.v1.pdf"), height=7, width=14)
  op = par(mfrow=c(1,2))
  for(mut in mutation2plot) {
    for(ng in ng_seq2plot) {
  ...
  pdf(file=file.path(resdir, "FIGURE2.LoD_PLOTS_INDEL.G360CDx.74genes.v1.pdf"), height=7, width=14)
  op = par(mfrow=c(1,2))
  for(mut in mutation2plot) {
    for(ng in ng_seq2plot) {
  ...
  pdf(file=file.path(resdir, "FIGURE2.LoD_PLOTS_CNV.G360CDx.74genes.v1.pdf"), height=7, width=14)
  op = par(mfrow=c(1,2))
  for(mut in mutation2plot) {
    for(ng in ng_seq2plot) {
  ...
  pdf(file=file.path(resdir, "FIGURE2.LoD_PLOTS_FUSION.G360CDx.74genes.v1.pdf"), height=7, width=14)
  op = par(mfrow=c(1,2))
  for(mut in mutation2plot) {
    for(ng in ng_seq2plot) {
  ...
  #load(file=file.path(resdir, "glm.models.LoD.G360CDx.74genes.200128.RData"))
  ...
  ##  SNVs
  tab.snv = read.table(file=file.path(resdir, "Table5.LoD.part2.G360CDx.74genes.tsv"), sep="\t", header=T, stringsAsFactors = F) %>%
  ...
  tab.snv.agg = read.table(file=file.path(resdir, "Table5.LoD.part1.G360CDx.74genes.tsv"), sep="\t", header=T, stringsAsFactors = F) %>%
                filter(variant_type == "SNV") %>%
                mutate(mut_key = variant, lod.maf = lod.probit,   lod.FINAL = LoD) %>%  
                dplyr::select("mut_key", "Input", "lod.probit", "nValid", "level_maf", "lod.maf", "lod.FINAL")
  ...
  ##  INDELS
  tab.indel = read.table(file=file.path(resdir, "Table5.LoD.part2.G360CDx.74genes.tsv"), sep="\t", header=T, stringsAsFactors = F) %>%
    filter(variant_type == "INDEL") %>%
    mutate(mut_key = variant, lod.maf = lod.probit,   lod.FINAL = LoD) %>%  
    dplyr::select("mut_key", "Input", "lod.probit", "nValid", "level_maf", "lod.maf", "lod.FINAL")
  tab.indel.agg = read.table(file=file.path(resdir, "Table5.LoD.part1.G360CDx.74genes.tsv"), sep="\t", header=T, stringsAsFactors = F) %>%
    filter(variant_type == "INDEL") %>%
    mutate(mut_key = variant, lod.maf = lod.probit,   lod.FINAL = LoD) %>%  
    dplyr::select("mut_key", "Input", "lod.probit", "nValid", "level_maf", "lod.maf", "lod.FINAL")
  ...
  ##  CNVs
  tab.cnv = read.table(file=file.path(resdir, "Table5.LoD.G360CDx.74genes.tsv"), sep="\t", header=T, stringsAsFactors=F) %>% 
            filter(variant_type == "CNV") %>%
            mutate(mut_key = variant, lod.maf = lod.probit,   lod.FINAL = LoD) %>%  
            dplyr::select("mut_key", "Input", "lod.probit", "nValid", "level_maf", "lod.maf", "lod.FINAL")
  
  ##  FUSIONS
  tab.fusion = read.table(file=file.path(resdir, "Table5.LoD.G360CDx.74genes.tsv"), sep="\t", header=T, stringsAsFactors=F) %>% filter(variant_type == "FUSION") %>%
    mutate(mut_key = variant, lod.maf = lod.probit,   lod.FINAL = LoD) %>%  
    dplyr::select("mut_key", "Input", "lod.probit", "nValid", "level_maf", "lod.maf", "lod.FINAL")
  
  tab.all = rbind(tab.snv, tab.indel, tab.cnv, tab.fusion)
  write.table(tab.all, file=file.path(resdir, "Table5.LoD.VARIANTSL.G360CDx.74genes.tsv"), sep="\t", col.names=T, row.names=F, quote = F)
  
  cat("DONE LoD G360CDx method for 74 genes..\n")
